...
public class LoginController {
	...
	private String userNameSmartbi;

	@Value("${smartbi.password}")
	private String passwordSmartbi;
	...
	public Result<JSONObject> login(@RequestBody SysLoginModel sysLoginModel,HttpServletRequest request) throws Exception {
		...
		String cookie_encode = "";
		ClientConnector conn = new ClientConnector(urlSmartbi);
		try {
			/*String user = "huxingying";
			String password = "123456";
			String smartbiURL = "http://106.14.195.28/smartbi";*/

			// 建立此连接时，就对smartbi进行了登录
			boolean ret = conn.open(userNameSmartbi, passwordSmartbi);
			if (ret) {
				String cookie = conn.getCookie();
				cookie_encode = java.net.URLEncoder.encode(cookie, "UTF-8");
				log.info("smartbi cookid ={}", cookie_encode);
			}
		}catch (Exception e) {
			e.printStackTrace();
		}finally {
		...
		// 将connect 放入session
		request.getSession().setAttribute("smartbi_connect",conn);
		request.getSession().setAttribute("smartbi_cookie",cookie_encode);
		return result;
	}
	...
	public Result<Object> logout(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			ClientConnector conn = (ClientConnector)  request.getSession().getAttribute("smartbi_connect");
			if(conn != null) {
				conn.close();
				log.info("smartbi connect is closed");
			}
