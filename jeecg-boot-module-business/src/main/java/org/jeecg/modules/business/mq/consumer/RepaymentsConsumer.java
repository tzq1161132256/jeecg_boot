 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_REPAYMENTS)
@Slf4j
public class RepaymentsConsumer {
  //  private final Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
  IRepaymentsService repaymentsService;
    ...
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            Repayments model = JSON.parseObject(sendMsg, Repayments.class);
            model.setUpdateTime(new Date());
            model.setCreateTime(new Date());
            repaymentsService.saveOrUpdate(model);
