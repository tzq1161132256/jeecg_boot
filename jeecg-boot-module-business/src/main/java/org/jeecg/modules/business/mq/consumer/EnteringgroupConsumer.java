 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_ENTERINGGROUP)
@Slf4j
public class EnteringgroupConsumer {
  //  private final Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
  IEnteringgroupService enteringgroupService;
    ...
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            Enteringgroup model = JSON.parseObject(sendMsg, Enteringgroup.class);
            model.setUpdateTime(new Date());
            model.setCreateTime(new Date());
            enteringgroupService.saveOrUpdate(model);
