 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_POS_TRANSACTION)
@Slf4j
  ...
  IPostransactionService postransactionService;
    ...
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            Postransaction model = JSON.parseObject(sendMsg, Postransaction.class);
         //   model.setUpdateTime(new Date());
          //  model.setCreateTime(new Date());
            postransactionService.processPostransaction(model);
