 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_INSURANCE_ZHAOSHANGRENHE)
@Slf4j
public class InsuranceZhaoshangrenheConsumer {
  //  private final Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
  IInsuranceZhaoshangrenheService zhaoshangrenheService;
    ...
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            InsuranceZhaoshangrenhe model = JSON.parseObject(sendMsg, InsuranceZhaoshangrenhe.class);
            model.setCreateTime(new Date());
            model.setUpdateTime(new Date());
            zhaoshangrenheService.saveOrUpdate(model);
