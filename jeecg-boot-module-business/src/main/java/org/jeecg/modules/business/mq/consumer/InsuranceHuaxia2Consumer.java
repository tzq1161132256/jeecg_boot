 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_INSURANCE_HUAXIA2)
@Slf4j
public class InsuranceHuaxia2Consumer {
  //  private final Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
  IInsuranceHuaxia2Service huaxia2Service;
    ...
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            InsuranceHuaxia2 model = JSON.parseObject(sendMsg, InsuranceHuaxia2.class);
            model.setCreateTime(new Date());
            model.setUpdateTime(new Date());
            huaxia2Service.saveOrUpdate(model);
