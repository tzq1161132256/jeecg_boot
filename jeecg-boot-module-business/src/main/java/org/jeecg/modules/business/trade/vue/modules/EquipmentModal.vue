    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="App版本号">
          <a-input placeholder="请输入App版本号" v-decorator="['appversion', {}]" />
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
          label="最后使用时间（最后交易时间）">
          <a-date-picker showTime format='YYYY-MM-DD HH:mm:ss' v-decorator="[ 'lastusetime', {}]" />
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/trade/equipment/add",
          edit: "/trade/equipment/edit",
        },
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'drugstorename','provincename','cityname','equipmenttype','equipmentcode','equipmentname','equipmentid','appversion','remark1','remark2','drugstoreid','delFlag'))
		  //时间格式化
          this.form.setFieldsValue({lastusetime:this.model.lastusetime?moment(this.model.lastusetime):null})
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            //时间格式化
            formData.lastusetime = formData.lastusetime?formData.lastusetime.format('YYYY-MM-DD HH:mm:ss'):null;
