 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_INSURANCE_HUAXIA)
@Slf4j
public class InsuranceHuaxiaConsumer {
  //  private final Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
  IInsuranceHuaxiaService insuranceHuaxiaService;
    ...
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            InsuranceHuaxia model = JSON.parseObject(sendMsg, InsuranceHuaxia.class);
            model.setUpdateTime(new Date());
            model.setCreateTime(new Date());
            insuranceHuaxiaService.processInsuraceHuaxia(model);
