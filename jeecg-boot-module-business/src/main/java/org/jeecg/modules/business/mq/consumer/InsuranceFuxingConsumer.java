 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_INSURANCE_FUXING)
@Slf4j
public class InsuranceFuxingConsumer {
  //  private final Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
  IInsuranceFuxingService fuxingService;
    ...
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            InsuranceFuxing model = JSON.parseObject(sendMsg, InsuranceFuxing.class);
            model.setCreateTime(new Date());
            model.setUpdateTime(new Date());
            fuxingService.saveOrUpdate(model);
