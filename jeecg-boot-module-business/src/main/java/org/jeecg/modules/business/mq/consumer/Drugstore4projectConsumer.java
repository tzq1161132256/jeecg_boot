 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_DRUGSTORE4PROJECT)
@Slf4j
public class Drugstore4projectConsumer {
  ...
  IDrugstore4projectService drugstore4projectService;
    ...
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            Drugstore4project model = JSON.parseObject(sendMsg, Drugstore4project.class);
            model.setUpdateTime(new Date());
            model.setCreateTime(new Date());
            drugstore4projectService.processDrugstore4project(model);
