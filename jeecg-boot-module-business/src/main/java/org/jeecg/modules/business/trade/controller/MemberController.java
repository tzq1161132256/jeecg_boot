...
public class MemberController {
	...
	public Result<Member> edit(@RequestBody Member member) {
		Result<Member> result = new Result<Member>();
	...
	public Result<Member> delete(@RequestParam(name="id",required=true) String id) {
		Result<Member> result = new Result<Member>();
		Member member = memberService.getById(id);
		if(member==null) {
			result.error500("未找到对应实体");
		}else {
			boolean ok = memberService.removeById(id);
			if(ok) {
				result.success("删除成功!");
			}
		}
	...
	public Result<Member> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		Result<Member> result = new Result<Member>();
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      ...
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "会员信息列表");
      mv.addObject(NormalExcelConstants.CLASS, Member.class);
      mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("会员信息列表数据", "导出人:Jeecg", "导出信息"));
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
          ...
          try {
              ...
              for (Member memberExcel : listMembers) {
                  memberService.save(memberExcel);
              }
