 ...
 */
@Component
@RabbitListener(queues = RabbitConfig.QUEUE_INSURANCE_DETAIL)
@Slf4j
public class InsuranceDetailConsumer {
  //  private final Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
    IInsuranceDetailService insuranceDetailService;
    @RabbitHandler
    public void process(String sendMsg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            InsuranceDetail model = JSON.parseObject(sendMsg, InsuranceDetail.class);
            model.setUpdateTime(new Date());
            model.setCreateTime(new Date());
            insuranceDetailService.processInsuranceDetail(model);
